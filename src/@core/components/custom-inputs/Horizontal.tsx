...
const CustomInputHorizontal = (props: CustomInputHorizontalProps) => {
  ...
  const renderData = () => {
    if (meta && title && content) {
      return (
        <div className='flex flex-col gap-1.5 bs-full is-full'>
          <div className='flex items-start justify-between gap-1.5 is-full mbs-1.5'>
      ...
      return (
        <div className='flex flex-col gap-1.5 bs-full mbs-1.5'>
          {typeof title === 'string' ? <Title>{title}</Title> : title}
          {typeof content === 'string' ? <Content>{content}</Content> : content}
        </div>
      )
