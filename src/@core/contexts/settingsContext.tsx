...
type SettingsCookie = Pick<Settings, 'mode' | 'layout'>
...
export const SettingsProvider = (props: Props) => {
  ...
  // Cookies (only mode and layout)
  const [settingsCookie, updateSettingsCookie] = useObjectCookie<SettingsCookie>(
    themeConfig.settingsCookieName,
    JSON.stringify(props.settingsCookie) !== '{}'
      ? props.settingsCookie
      : { mode: updatedInitialSettings.mode, layout: updatedInitialSettings.layout }
  )
  ...
  const updateSettings = (settingsCookie: Partial<SettingsCookie>, options?: UpdateSettingsOptions) => {
    const { updateCookie = true } = options || {}

    _updateSettingsState(prev => {
      const updated: SettingsCookie = {
        layout: settingsCookie.layout || prev.layout,
        mode: settingsCookie.mode || prev.mode
      }

      // Update cookie if needed
      if (updateCookie) updateSettingsCookie(updated)
   ...
   * }, []);
   */
  const updatePageSettings = (settings: Partial<SettingsCookie>): (() => void) => {
