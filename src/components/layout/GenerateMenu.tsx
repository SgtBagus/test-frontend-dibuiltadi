...
export const GenerateVerticalMenu = ({ menuData }: { menuData: VerticalMenuDataType[] }) => {
  ...
  const renderMenuItems = (data: VerticalMenuDataType[]): ReactNode => {
    ...
    return data.map((item: VerticalMenuDataType, index) => {
      ...
      if (subMenuItem.children) {
        const { children, icon, prefix, suffix, className, ...rest } = subMenuItem

        if (isCollapsed) {
          return (
            <MenuSection key={index} {...rest} className={cn('mt-2', className)}>
        ...
        if (!subMenuItem.level || subMenuItem.level === 0) {
          return (
            <div key={index} className='mt-2'>
              <VerticalSubMenu
                ...
                {...rest}
                menukey={rest.menukey}
                {...(Icon && { icon: Icon })}
              >
                {children && renderMenuItems(children.map(c => ({ ...c, level: (c.level ?? 0) + 1 })))}
              </VerticalSubMenu>
            </div>
          )
        }
      ...
      const { label, icon, prefix, suffix, className, ...rest } = menuItem
