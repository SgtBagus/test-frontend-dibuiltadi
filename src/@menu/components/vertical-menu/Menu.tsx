...
import { VerticalMenuItemDataType, VerticalSectionDataType } from '@/types/menuTypes'
...
export type MenuProps = VerticalMenuContextProps &
  ...
  MenuHTMLAttributes<HTMLMenuElement> & {
    ...
    menuItems: (VerticalSectionDataType | VerticalMenuItemDataType)[]
  }
...
const Menu: ForwardRefRenderFunction<HTMLMenuElement, MenuProps> = (props, ref) => {
  ...
  const [openSubmenu, setOpenSubmenu] = useState<OpenSubmenu[]>(() =>
    menuItems.map(item => ({ level: 0, active: false, label: item.label, id: item.menukey }))
  )
  ...
  return (
    <VerticalMenuContext.Provider value={providerValue}>
      <FloatingTree>
        <StyledVerticalMenu ref={ref} className={cn(menuClasses.root, className)} rootStyles={rootStyles} {...rest}>
